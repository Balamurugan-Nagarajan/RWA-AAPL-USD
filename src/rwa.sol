...
/**
 * @title dTSLA
 * @notice This is our contract to make requests to the Alpaca API to mint aapl-backed dTSLA tokens
...
contract dTSLA is FunctionsClient, ConfirmedOwner, ERC20, Pausable {
    ...
    /*
     * @notice user sends a Chainlink Functions request to sell aapl for redemptionCoin
     * @notice this will put the redemptionCoin in a withdrawl queue that the user must call to redeem
     * 
     * @dev Burn dTSLA
     * @dev Sell aapl on brokerage
    ...
    // aapl USD has 8 decimal places, so we add an additional 10 decimal places
